...
- `no-mistakes-prod-only` is a conditional policy rather than one flat mode, and AGENTS.md section 7 owns its per-task surface classification.
...
Registering a conditional policy is a one-time choice and never requires classifying any change at registration.
...
A `no-mistakes` or `no-mistakes-prod-only` project must have an `origin` remote and must complete the initialization procedure below, because the conditional policy may require the pipeline for a later task.
