...
| Launch | A positional prompt with a help-probed workspace trust path, `--yolo`, `--model <model>` when selected, and `--workspace <absolute-task-worktree>`, behind `env -u` of the foreign primary markers. Builds whose help allows interactive trust keep `--trust` on the TTY launch; builds whose help says `--trust` is print/headless-only first run `cursor-agent --trust --workspace <worktree> create-chat` and then omit `--trust` from the interactive TTY launch. |
...
| Trust dialog | Repository trust suppresses it. `--yolo` does NOT, and every task gets a fresh worktree path, so `fm-spawn.sh` refuses when it cannot prove either the old interactive `--trust` contract or the current headless preflight contract. |
...
| Primary limits | `stop` does not fire in headless `cursor-agent -p`. `preCompact` is deliberately unregistered because it cannot inject context, so a Cursor primary does not re-emit its digest after a compaction; that surface is deferred to a follow-up. Project hooks need a trusted project workspace. |
