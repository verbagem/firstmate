...
- `TOOL_AUTOUPDATE: <diagnostic>` - the opt-in `config/tool-autoupdate` sweep (`docs/configuration.md` "Tool auto-update") could not complete for one package: a failed `npm install -g <pkg>@latest`, a failed registry lookup, an unreachable `npm`/network, or a timed-out check.
