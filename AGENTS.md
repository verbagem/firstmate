...
.env                 optional Relay pairing token (presence-gates section 14) and typed dispatch resolution key TYPESAFE_API_KEY (presence-gates bin/fm-dispatch-resolve.sh; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
config/tool-autoupdate  optional presence flag enabling default-off unattended `npm install -g <pkg>@<version>` for a fixed set of already-installed axi-family and Kun Chen tools (never treehouse or no-mistakes); the publish-age gate for that curated list defaults to 0 (captain-trusted), overridable per home via FM_TOOL_AUTOUPDATE_MIN_AGE_DAYS; LOCAL, gitignored, not inherited; see docs/configuration.md "Tool auto-update" and bin/fm-bootstrap.sh's header
...
state/               runtime records and signals; gitignored
  ...
  .tool-autoupdate-checked  24h throttle stamp for the opt-in config/tool-autoupdate sweep; safe to delete (forces one re-check)
...
Every network check a session start owes - GitHub auth, dead-secondmate relaunch, secondmate convergence, pending handoff delivery, project clone refresh, and the opt-in tool auto-update sweep - runs off the digest's blocking path in a bounded worker owned by `bin/fm-startup-network.sh` and is reported in the digest's own `NETWORK CHECKS` section.
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the six bootstrap MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, and Relay artifact writes - plus the opt-in `config/tool-autoupdate` sweep run only when this session actually holds the lock from step 1; the four network ones among them, and that opt-in sweep, run in the deferred stage rather than in this section.
...
When dispatch profiles exist, consult them at every crewmate or scout intake; `fm-spawn` realizes an opted-in clear typed selection and requires a deterministic reason for divergence while retaining final launch validation.
`docs/configuration.md` "Typed Dispatch Resolution" owns the optional `TYPESAFE_API_KEY` resolver, private receipt, and fallback contract; absent the key or on a non-clear result, the existing intake remains authoritative.
Routing precedence is an explicit per-task captain override, then the best-fit configured rule, then the configured default, then the static crewmate harness.
On the manual path, Firstmate alone resolves a matched profile array: begin with `quota-axi`'s default TOON at that intake, using the skill's narrow TOON-then-`--json` fallback only for genuine ambiguity, evaluate every configured candidate against that current output, and choose with inspectable `spendPriority` as the one quota-perspective ranker after the skill's eligibility, reasoning-class, and runway-feasibility gates.
...
Use immediate steering for corrections, blockers, clarifications of already accepted intent, or requirements that invalidate the current path; validation-time additive requests follow the section 7 follow-up boundary instead.
...
Once validation starts, additive requests default to durable follow-up work under section 10 rather than interrupting accepted work already under validation.
Keep only corrections, blockers, clarifications of already accepted intent, or requirements that completely invalidate the work being validated with the current task.
...
Agent-initiated recommendations that ask the captain to approve, reject, or choose a next action use the proposal-card contract owned by `bin/fm-proposal-card.sh`; use that owner for decision-time classes and the optional rejection-reason path.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `PR_CHECK_MIGRATION:`, `HOME_SUMMARY:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, `TOOL_AUTOUPDATE:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
- `diagnostic-reasoning` - load before scoping a reported bug and before acting on a diagnostic report.
- `ask-user-authority` - load before deciding any ask-user finding.
- `exceptional-worker-graph` - load before requesting or acting on a specialist planner/adjudicator graph for exceptional complex work, including any Fable 5.1 use; routine work and primary Pi supervision stay on existing dispatch paths.
