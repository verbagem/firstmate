...
`docs/configuration.md` "Typed Dispatch Resolution" owns the optional `TYPESAFE_API_KEY` resolver path; absent the key, dispatch behavior is unchanged, any non-clear resolver outcome returns to this intake, and a clear profile remains subject to Firstmate judgment, captain approval, local provider/catalog evidence, and `fm-spawn` validation.
