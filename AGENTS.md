...
state/               runtime records and signals; gitignored
  ...
  .session-start-digest.<pid>.txt  per-Pi-process session-start digest the extension points at instead of inlining; overwritten each open, swept by a later process's write once its pid is gone; docs/sessionstart-nudge.md
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .run-state-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
