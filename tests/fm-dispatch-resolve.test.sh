...
OPENCODE_RULE="$TMP_ROOT/opencode-rule.json"
printf '%s\n' '{"rules":[{"when":"OpenCode work.","use":{"harness":"opencode","model":"anthropic/claude-sonnet-4-5","provider":"google"}}]}' > "$OPENCODE_RULE"
cp "$OPENCODE_RULE" "$RULES"
cat > "$RESPONSE" <<'JSON'
...
reset_log
TYPESAFE_API_KEY=$KEY run code out err "$BRIEF"
assert_contains "$out" 'candidate: opencode:anthropic/claude-sonnet-4-5  provider=google  scope=all_models  remaining=72%  spendPriority=0.3  runway=through_reset  -> eligible' "verified multi-provider harnesses resolve through explicit provider"
assert_contains "$out" "  profile: --harness 'opencode' --model 'anthropic/claude-sonnet-4-5'" "explicit-provider OpenCode rule resolves"
...
cp "$BASE_RULES" "$RULES"
pass "no-rule fallback, explicit provider, and documented configurations resolve"
