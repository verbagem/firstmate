...
A newer release still younger than `FM_TOOL_AUTOUPDATE_MIN_AGE_DAYS` (default 14, matching the standing wait-before-installing rule) is left alone and picked up on a later run once it clears the window; opting in authorizes auto-updating once a release has aged past that gate, never installing it same-day.
