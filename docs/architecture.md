...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working and carry new captain-facing information whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A durable `captain-held` endpoint uses that bounded cadence only after the backend confidently reports its agent dead, while live or inconclusive held liveness still surfaces once; a secondmate's endpoint liveness is still never read at all, and a mate is admitted to that same cadence only to serve a declared wait's bounded re-surface, so a forgotten pause or captain hold on a mate cannot rot invisibly.
...
By default, the dispatch file remains judgment-based: firstmate reads the natural-language rules at intake, chooses the best matching rule, resolves profile arrays itself from current quota output under the `AGENTS.md` section 4 intake boundary and the `quota-array-dispatch` selection procedure, and passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
The optional typed resolver path is owned by [configuration.md](configuration.md#typed-dispatch-resolution-env-typesafe_api_key); without `TYPESAFE_API_KEY`, shell scripts still do not parse task intent or match natural-language rules, and with it `fm-dispatch-resolve.sh` remains a bounded clear/non-clear helper rather than another router or control plane.
The remaining shell scripts validate the JSON shape and verified harness/effort combinations, but they do not parse task intent, match natural-language rules, or own array selection.
...
When the file exists, `fm-spawn.sh` stops `config/crew-harness` from acting as an automatic crewmate/scout fallback; a clear typed selection may supply the profile automatically, and other outcomes require an explicit resolved harness with supported fallback handling.
