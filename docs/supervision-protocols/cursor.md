...
That document owns the double loop bound, the supersession contract, the Pi-host stand-down, and the compatibility limits, including that a Cursor primary must satisfy Cursor's help-probed workspace-trust contract for its project hooks to load at all.
