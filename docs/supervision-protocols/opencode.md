...
3. The plugin listens for `session.idle`, awaits the `bin/fm-watch-arm.sh --restart` launch in the idle handler until the watcher reports ready or the bounded readiness wait elapses, and owns every later successor launch.
