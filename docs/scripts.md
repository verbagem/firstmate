...
| `fm-jev-evidence-screen.sh` | Evaluate advisory-only evidence packets into bounded review and test-selection receipts |
| `fm-jev-browser-shadow.sh` | Run the bounded fixture-only Jev browser-action selector pilot and emit recommendation receipts |
...
| `fm-line-cap-lib.sh`     | Shared per-line truncation helper for agent-facing status and decision digests       |
...
| `fm-quota-axi-lib.sh`    | Shared `quota-axi` floor, schema validator, and resolver single-provider harness table |
| `fm-vendor-auth-probe.sh`| Run one hard-bounded, non-destructive authentication probe of a named vendor CLI and report the fact |
| `fm-local-inference.sh`  | Inspect opt-in local-inference catalogs, hardware fit, deterministic admission, and loopback model discovery |
...
| `fm-pi-recap.sh`         | Render the sanitized captain-facing task recap (title, phase, open blocker/decision, PR) for the Pi `fm-task-recap` widget |
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode |
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees, require completed scout deliverables, retire secondmate homes |
| `fm-harness.sh`          | Detect the running harness and resolve crew or secondmate harness, model, and effort |
| `fm-lock.sh`             | Per-home firstmate session lock                                                      |
| `fm-env-lib.sh`          | Shared one-key `.env` accessor for opt-in local secrets                              |
